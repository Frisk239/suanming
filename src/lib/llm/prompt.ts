...
const DEPTH_DESC: Record<string, string> = {
  ...
  popular: `通俗级（面向完全不懂命理的普通人）：
- 像跟朋友聊天那样解释，语气轻松，不端着，不用「综上」「可见」「值得注意」这类报告腔
- 专业术语第一次出现必须用日常比喻解释：如「七杀格」不说七杀格，说「你这命局像一把没入鞘的快刀，锋利但容易伤己」
- 能用比喻/类比/生活场景就用，别术语堆砌：「印星」不如「贵人/靠山」，「调候」不如「夏天出生的命需要点水降温」
- 宁可啰嗦也要让人懂，不为简洁牺牲理解
- 结论直给，但要有「为什么会这样」的白话铺垫`,
};
